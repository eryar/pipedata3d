...
set aDescription "Long Radius Elbow 45 Degree"
...
proc buildChildren {theStandardTree theParentNode theId theSource} {
    #puts "Long Radius Elbow 45 Degree build children $theId..."
    foreach item $::aDimension {
        incr theId
        $theStandardTree insert $theParentNode end -text NPS-[lindex $item 0] -tags "LR45D$theId"
        $theStandardTree tag bind LR45D$theId <1> "selectedItem {$theSource} {$::aHeading} {$item}"
    }
}
...
proc buildModel {theDimension} {
    ...
    vpoint p1 0 0 0
    vpoint p2 $A 0 0
    vdimension dim1 -length -plane xoy -shapes p1 p2 -flyout -15 -color black
    vdimension dim2 -diameter -plane xoy -shapes E_2 -flyout -15 -color black
